...
describe("gateway tests", () => {
  describe("provision integrator", () => {
    ...
    it("should confirm mock calls", () => {
      expect(kongMock.kongEnsureConsumer.mock.calls[0][0]).toEqual(
        "project12345"
      );
      expect(kongMock.kongEnsureRequestTransformer.mock.calls[0][0]).toEqual(
        "project12345"
      );
      expect(kongMock.kongEnsureRequestTransformer.mock.calls[0][1]).toEqual(
        "integrator12345"
      );
      expect(kongMock.kongEnsureAcl.mock.calls[0][0]).toEqual("project12345");
      expect(kongMock.kongEnsureAcl.mock.calls[0][1]).toEqual(
        "swap_v1_price_group"
      );
      expect(kongMock.kongEnsureRateLimit.mock.calls[0][0]).toEqual(
        "project12345"
      );
      expect(kongMock.kongEnsureRateLimit.mock.calls[0][1]).toEqual(
        "swap_price_v1_route_optimism"
      );
      expect(kongMock.kongEnsureRateLimit.mock.calls[0][2]).toEqual({
        minute: 30,
      });
      expect(kongMock.kongEnsureRateLimit.mock.calls[1][1]).toEqual(
