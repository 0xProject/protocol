 ...
 */
export async function kongGetConsumer(
  projectId: string
): Promise<KongConsumer | null> {
  try {
    const { data, status } = await kongGet<KongConsumer>(
      `/consumers/${projectId}`
    );
 ...
 * @param projectId Kong consumer projectId
 */
export async function kongEnsureConsumer(
  projectId: string
): Promise<KongConsumer | null> {
  const { data, status } = await kongPut<KongConsumer>(
    `/consumers/${projectId}`
  );
  return status === StatusCodes.OK ? data : null;
}
 ...
 */
export async function kongEnsureRequestTransformer(
  projectId: string,
  integratorId: string
): Promise<boolean> {
  let foundPlugin;
  try {
    const { data, status } = await kongGet<KongPlugins>(
      `/consumers/${projectId}/plugins`
    );
    if (status !== StatusCodes.OK) {
      return false;
    }
    foundPlugin = data.data.find(
 ...
 */
export async function kongGetKey(
  projectId: string,
  key: string
): Promise<KongKey | null> {
  try {
    const { data, status } = await kongGet<KongKeys>(
      `/consumers/${projectId}/key-auth`
    );
    if (status !== StatusCodes.OK) {
      return null;
    }
    return (
      (data.data.find((keyInfo) => keyInfo.key === key) as KongKey) || null
    );
  } catch {
    return null;
  }
 ...
 * @param key Kong API key
 */
export async function kongEnsureKey(
  projectId: string,
  key: string
): Promise<KongKey | null> {
  const existingKey = await kongGetKey(projectId, key);
  if (existingKey) {
    const { data, status } = await kongPut<KongKey>(
      `/consumers/${projectId}/key-auth/${existingKey.id}`,
      { key }
    );
    return status !== StatusCodes.OK ? null : data;
  } else {
    const { data, status } = await kongPost<KongKey>(
      `/consumers/${projectId}/key-auth`,
      { key }
    );
 ...
 * @param key Kong API key
 */
export async function kongRemoveKey(
  projectId: string,
  key: string
): Promise<boolean> {
  try {
    ...
    const { status } = await kongDelete(
      `/consumers/${projectId}/key-auth/${kongKey.id}`
    );
    return status === StatusCodes.NO_CONTENT;
  } catch {
    return false;
 ...
 */
export async function kongGetAcl(
  projectId: string,
  group: string
): Promise<KongAcl | null> {
  try {
    const { data, status } = await kongGet<KongAcl>(
      `/consumers/${projectId}/acls/${group}`
    );
    return status === StatusCodes.OK ? data : null;
  } catch {
    return null;
 ...
 * @param group Kong ACL group
 */
export async function kongEnsureAcl(
  projectId: string,
  group: string
): Promise<KongAcl | null> {
  const { data, status } = await kongPut<KongAcl>(
    `/consumers/${projectId}/acls/${group}`
  );
  return status === StatusCodes.OK ? data : null;
}
 ...
 * @param group Kong ACL group
 */
export async function kongRemoveAcl(
  projectId: string,
  group: string
): Promise<boolean> {
  try {
    ...
    const { status } = await kongDelete(
      `/consumers/${projectId}/acls/${kongAcl.id}`
    );
    return status === StatusCodes.NO_CONTENT;
  } catch {
    return false;
...
export async function kongEnsureRateLimit(
    ...
    redisConfig = {
      ...
      redis_database:
        redisUrl.pathname.length > 1
          ? parseInt(redisUrl.pathname.substring(1))
          : 0,
  ...
  const req = {
    ...
    config: {
      ...
      ...redisConfig,
      error_message:
        "Rate limit exceeded - see https://www.0x.org/rate-limit for more details",
    },
  };
  ...
  if (existingRateLimit) {
    ...
    const { data, status } = await kongPost<KongPlugin<ZippoRateLimit>>(
      `/consumers/${projectId}/plugins`,
      req
    );
    return status !== StatusCodes.CREATED ? null : data;
 ...
 */
export async function kongRemoveRateLimit(
  projectId: string,
  routeName: string
): Promise<boolean> {
  try {
    ...
    const { status } = await kongDelete(
      `/consumers/${projectId}/plugins/${kongRateLimit.id}`
    );
    return status === StatusCodes.NO_CONTENT;
  } catch {
    return false;
...
function kongPost<T, R extends Record<string, unknown> = {}>(
  path: string,
  req: R
...
function kongPut<T, R extends Record<string, unknown> = {}>(
  path: string,
  req?: R
): Promise<AxiosResponse<T>> {
