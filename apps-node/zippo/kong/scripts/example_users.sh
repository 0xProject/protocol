...
curl -i -X POST http://127.0.0.1:8001/consumers/alice-p1/plugins --data name=request-transformer --data "config.rename.headers=X-Consumer-Username:0x-App-Id" --data "config.add.headers=0x-Integrator-Id:alice"
...
curl -i -X POST http://127.0.0.1:8001/consumers/bob-p1/plugins --data name=request-transformer --data "config.rename.headers=X-Consumer-Username:0x-App-Id" --data "config.add.headers=0x-Integrator-Id:bob"
# bob app 2 - orderbook
...
curl -i -X POST http://127.0.0.1:8001/consumers/bob-p2/plugins --data name=request-transformer --data "config.rename.headers=X-Consumer-Username:0x-App-Id" --data "config.add.headers=0x-Integrator-Id:bob"
...
curl -i -X POST http://127.0.0.1:8001/consumers/charlie-p1/plugins --data name=request-transformer --data "config.rename.headers=X-Consumer-Username:0x-App-Id" --data "config.add.headers=0x-Integrator-Id:charlie"
