...
APP_ID=cle7j1k1q000g07mhc9ti587w
...
# create consumer
curl -i -X PUT http://127.0.0.1:8901/consumers/$APP_ID
# setup header transforms for consumer
curl -i -X POST http://127.0.0.1:8901/consumers/$APP_ID/plugins --data name=request-transformer --data "config.rename.headers=X-Consumer-Username:0x-App-Id" --data "config.add.headers=0x-Integrator-Id:$INTEGRATOR_ID"

# assign API key(s) to consumer
curl -i -X POST http://127.0.0.1:8901/consumers/$APP_ID/key-auth --data key=$API_KEY

# enable route access to consumer
curl -i -X POST http://127.0.0.1:8901/consumers/$APP_ID/acls --data "group=$ACL_GROUP_NAME"
# assign rate limit for route for consumer
curl -i -X POST http://127.0.0.1:8901/consumers/$APP_ID/plugins --data name=rate-limiting --data "config.minute=5" --data "route.name=$ROUTE_NAME" --data "config.error_message=$RATE_LIMIT_ERROR_MSG" --data "config.redis_host=$REDIS_HOST" --data "config.policy=redis" --data "config.redis_timeout=1000" --data "config.redis_ssl=true"
