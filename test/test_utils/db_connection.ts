 ...
 */
export async function initDBConnectionAsync(port: number = 5432): Promise<Connection> {
    const connection = await getDBConnectionAsync(`postgres://api:api@localhost:${port}/api`);
