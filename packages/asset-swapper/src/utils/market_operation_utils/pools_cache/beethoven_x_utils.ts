...
export class BeethovenXPoolsCache extends PoolsCache {
    ...
    constructor(
        chainId: ChainId,
        private readonly subgraphUrl: string = BEETHOVEN_X_SUBGRAPH_URL_BY_CHAIN[chainId],
                ...
                for (const to of tokensList.filter(t => t.toLowerCase() !== from.toLowerCase())) {
                    ...
                    try {
                        ...
                        fromToPools[from][to].push(
                            BeethovenXPoolsCache._parseSubgraphPoolData(poolData[pool.id], from, to),
                        );
    ...
    protected async _fetchPoolsForPairAsync(takerToken: string, makerToken: string): Promise<Pool[]> {
        ...
        try {
            ...
            return pools.map((pool: any) => BeethovenXPoolsCache._parseSubgraphPoolData(pool, takerToken, makerToken));
        } catch (e) {
            return [];
        }
    }
