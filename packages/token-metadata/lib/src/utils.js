...
function getTokenMetadataIfExists(tokenAddressOrSymbol, chainId) {
    ...
    if (isTokenAddress(tokenAddressOrSymbol)) {
        entry = token_metadata_1.TokenMetadatasForChains.find(tm => tm.tokenAddresses[chainId].toLowerCase() === tokenAddressOrSymbol.toLowerCase());
    }
    else {
        const normalizedSymbol = tokenAddressOrSymbol.toLowerCase();
        entry = token_metadata_1.TokenMetadatasForChains.find(tm => tm.symbol.toLowerCase() === normalizedSymbol);
    }
    if (entry) {
        return {
            symbol: entry.symbol,
...
function isNativeSymbolOrAddress(tokenSymbolOrAddress, chainId) {
    var _a;
    const nativeAddress = (_a = getTokenMetadataIfExists(nativeTokenSymbol(chainId), chainId)) === null || _a === void 0 ? void 0 : _a.tokenAddress;
...
function isNativeWrappedSymbolOrAddress(tokenAddressOrSymbol, chainId) {
    ...
    const wrappedAddress = (_a = getTokenMetadataIfExists(nativeWrappedTokenSymbol(chainId), chainId)) === null || _a === void 0 ? void 0 : _a.tokenAddress;
