    ...
    MultiplexUniswapV3
{
        ...
        returns (uint256 boughtAmount)
    {
        ...
        return _multiplexMultiHopSell(
            MultiHopSellParams({tokens: tokens, sellAmount: msg.value, calls: calls, useSelfBalance: true, recipient: msg.sender}),
            minBuyAmount
        );
    }
    ...
    {
        return _multiplexMultiHopSell(
            MultiHopSellParams({tokens: tokens, sellAmount: sellAmount, calls: calls, useSelfBalance: false, recipient: msg.sender}),
            minBuyAmount
        );
    }
