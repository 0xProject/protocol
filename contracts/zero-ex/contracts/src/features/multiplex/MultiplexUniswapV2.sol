...
abstract contract MultiplexUniswapV2 is FixinCommon, FixinTokenSpender {
        ...
        returns (uint256 boughtAmount)
    {
        ...
        require(
            address(this) != _implementation, "MultiplexLiquidityProvider::_batchSellUniswapV2External/ONLY_DELEGATECALL"
        );
        ...
        returns (address pairAddress)
    {
        ...
        if (isSushi) {
            ...
            return address(
                uint256(
                    keccak256(
                        abi.encodePacked(hex"ff", SUSHISWAP_FACTORY, keccak256(abi.encodePacked(token0, token1)), SUSHISWAP_PAIR_INIT_CODE_HASH)
                    )
                )
            );
        } else {
            ...
            return address(
                uint256(
                    keccak256(
                        abi.encodePacked(hex"ff", UNISWAP_FACTORY, keccak256(abi.encodePacked(token0, token1)), UNISWAP_PAIR_INIT_CODE_HASH)
                    )
                )
            );
        }
